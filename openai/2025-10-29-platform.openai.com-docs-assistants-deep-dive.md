...
The diagram below shows how a Run transitions between different states. While a Run is in_progress, it represents an active execution. Once it finishes, it moves into one of several terminal states — completed, failed, cancelled, or expired. These terminal states indicate the final outcome of the Run, mirroring the same lifecycle as Run statuses overall.
